...
private static final class Graph<T> {
	...
	public int[][]floydWarshal(){int[][]m=getAdjacencyMatrix();for(T k:nodes.keySet())for(T i:nodes.keySet())for(T j:nodes.keySet())if(m[(int)i][(int)k]!=Integer.MAX_VALUE&&m[(int)k][(int)j]!=Integer.MAX_VALUE)m[(int)i][(int)j]=Math.min(m[(int)i][(int)j],m[(int)i][(int)k]+m[(int)k][(int)j]);return m;}
	public int[][]getAdjacencyMatrix(){int[][]m=new int[numNodes][numNodes];for(int[]row:m)Arrays.fill(row,Integer.MAX_VALUE);for(T t:nodes.keySet()){m[(int)t][(int)t]=0;for(T c:nodes.get(t).edges.keySet())m[(int)t][(int)c]=nodes.get(t).edges.get(c);}return m;}
}
		...
		for (T k : nodes.keySet())
			for (T i : nodes.keySet())
				for (T j : nodes.keySet())
					if (m[(int) i][(int) k] != Integer.MAX_VALUE && m[(int) k][(int) j] != Integer.MAX_VALUE)
						m[(int) i][(int) j] = Math.min(m[(int) i][(int) j], m[(int) i][(int) k] + m[(int) k][(int) j]);
		return m;
	}
	...
	public int[][] getAdjacencyMatrix() {
		...
		for (T t : nodes.keySet()) {
			...
			for (T c : nodes.get(t).edges.keySet())
				m[(int) t][(int) c] = nodes.get(t).edges.get(c);
		}
		return m;
	}
